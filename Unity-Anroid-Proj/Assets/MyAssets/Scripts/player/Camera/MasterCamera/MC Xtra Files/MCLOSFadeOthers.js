/*
Add this script to any object allowing line of sight object fading on selectable layers.
...
function Awake(){
	camController = GameObject.Find("MasterCameraRig").GetComponent("MasterCamera") as MasterCamera;	
	...
	//camController.collisionLayerMask.value = combinedMask;
...
function Update () {
	...
	if(renderer.isVisible){
		var fadeCastDirection = transform.position - camController.cam.transform.position;
		Debug.DrawRay(camController.cam.transform.position, fadeCastDirection, Color.blue);
				...
				//do nothing
					...
					if (renderer[rl]) {
							...
							if(renderer[rl].material.HasProperty("_Color")){
								...
								//Placeholder value in the array for materials with no color such as Mobile > VertexLit
